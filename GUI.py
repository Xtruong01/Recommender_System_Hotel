...
def recommend_hotels_knn(user_id, num_recommendations=5):
    if user_id not in user_item_matrix.index:
        st.warning(f"Reviewer ID {user_id} không tồn tại. Sử dụng chiến lược dự phòng.")
        # Gợi ý các khách sạn phổ biến nhất
        popular_hotels = df_hotel_info.sort_values(by='Total_Score', ascending=False).head(num_recommendations)
        return popular_hotels[['Hotel_ID', 'Hotel_Name', 'Hotel_Rank', 'Hotel_Address', 'Total_Score']]
    
    try:
        # Tăng n_neighbors đã được điều chỉnh trước đó
        distances, indices = model_knn.kneighbors(user_item_matrix.loc[user_id].values.reshape(1, -1), n_neighbors=10)
        hotel_ids = user_item_matrix.columns[indices.flatten()][1:num_recommendations+1]  # Lấy đủ số lượng gợi ý
        recommended_hotels = df_hotel_info[df_hotel_info['Hotel_ID'].isin(hotel_ids)]
    except IndexError:
        st.error("Đã xảy ra lỗi khi tìm kiếm gợi ý cho người dùng này.")
        # Gợi ý dự phòng nếu có lỗi
        return df_hotel_info.sort_values(by='Total_Score', ascending=False).head(num_recommendations)
...
if choice == 'Gợi ý khách sạn dựa trên Content Based':  
